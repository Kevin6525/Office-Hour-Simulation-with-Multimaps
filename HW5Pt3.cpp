...
student::student(int arrival, int question)
{
	...
	priority = rand() % 10 + 1;
	nameGenerator = rand() % 10;
	subjectGenerator = rand() % 5;
}

void simulateOfficeHour(int questionTime, double probability, int totalTime, int& averageWait, int& averageQuestion, int& averageOver, int callNumber)
{
	...
	if(callNumber != 1)
	{
		outputFile.open("HW5Pt3output.txt", ios_base::app);
	}
	else
	{
	...
	for(currentTime = 1; currentTime <= totalTime; currentTime++)
	{
		...
		if(studentArrival.getProbability() && averageTime.totalStudents() <= 150)
		{
			...
			studentsTopics.insert(pair<string, string> (names[students.getName()], subjects[students.getSubject()]));
		}
	...
	outputFile<<"Office simulation #"<<callNumber<<endl;
	outputFile<<"\tName\tSubject"<<endl;
...
int main()
{
	...
	for(int i = 0; i < 100; i ++)
	{
		simulateOfficeHour(20, probability, 60, averageWait, averageQuestion, averageOffice, i+1);
